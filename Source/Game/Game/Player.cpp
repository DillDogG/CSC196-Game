...
void Player::Update(float dt) {
	...
	m_transform.position.x = kiko::Wrap(m_transform.position.x, (float)renderer.GetWidth());
	m_transform.position.y = kiko::Wrap(m_transform.position.y, (float)renderer.GetHeight());
}
